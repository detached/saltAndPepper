...
function recipeReducer(state, action) {
    switch (action?.type) {
        ...
        case EditRecipeActions.ADD_NEW_IMAGES:
            return {
                ...state,
                newImages: state.newImages ? state.newImages.concat(action.value) : action.value
            }
        case EditRecipeActions.REMOVE_NEW_IMAGE:
            return {
                ...state,
                newImages: state.newImages ? state.newImages.filter((image) => image !== action.value) : []
            }
        case EditRecipeActions.REMOVE_EXISTING_IMAGE:
            return {
                ...state,
                images: state.images ? state.images.filter((image) => image.id !== action.value.id) : []
            }
...
export default function EditRecipeForm({titelKey, initRecipe}) {
    ...
    const [recipe, recipeDispatcher] = useReducer(recipeReducer, initRecipe, recipeReducer);

    const [images, setImages] = useState(initRecipe.images ? initRecipe.images : []);
    ...
    const thumbs = images.map(file => (
        <div className="edit-recipe-images-thumb" key={file.name ? file.name : file.id}>
    ...
    const handleStoreRecipe = useCallback((e) => {
        e.preventDefault();
        if (isLoading) {
            return;
        }
        ...
        storeRecipe(recipe).then((result) => {
            ...
            <form className="pure-form pure-form-stacked">
                <div className="pure-u-1 pure-u-md-1-2">
                    <section className="edit-recipe-images-container">
                        <div {...getRootProps({className: 'edit-recipe-images-dropzone'})}>
                            <input {...getInputProps()} />
                ...
                <div className="pure-u-1 pure-u-md-1-2">
                    <fieldset>
                        ...
                        <input type="text" id="title" name="title"
                               placeholder={t("newRecipe.name")} className="pure-input-1"
                               value={recipe.title} onChange={e => recipeDispatcher({
                            type: EditRecipeActions.SET_TITLE,
                            value: e.target.value
                        })}/>
                        <label htmlFor="category">{t("recipe.category")}:</label>
                        <input type="text" id="category" name="category"
                               placeholder={t("recipe.category")} className="pure-input-1"
                               value={recipe.category} onChange={e => recipeDispatcher({
                            type: EditRecipeActions.SET_CATEGORY,
                            value: e.target.value
                        })}/>
                        <label htmlFor="cuisine">{t("recipe.cuisine")}:</label>
                        <input type="text" id="cuisine" name="cuisine"
                               placeholder={t("recipe.cuisine")} className="pure-input-1"
                               value={recipe.cuisine} onChange={e => recipeDispatcher({
                            type: EditRecipeActions.SET_CUISINE,
                            value: e.target.value
                        })}/>
                        <label htmlFor="yields">{t("recipe.yields")}:</label>
                        <input type="text" id="yields" name="yields"
                               placeholder={t("recipe.yields")} className="pure-input-1"
                               value={recipe.yields} onChange={e => recipeDispatcher({
                ...
                <div className="pure-u-1">
                    ...
                    <textarea id="ingredients" name="ingredients"
                              ...
                              value={recipe.ingredients} onChange={e => recipeDispatcher({
                    ...
                    <textarea id="instructions" name="instructions"
                              ...
                              value={recipe.instructions} onChange={e => recipeDispatcher({
                        type: EditRecipeActions.SET_INSTRUCTIONS,
                        value: e.target.value
                    })}></textarea>

                    <h2>{t("recipe.modifications")}</h2>
                    <textarea id="modifications" name="modifications" className="edit-recipe-textarea"
                              placeholder={t('recipe.modifications')}
                              value={recipe.modifications} onChange={e => recipeDispatcher({
                        type: EditRecipeActions.SET_MODIFICATIONS,
                        value: e.target.value
                    })}></textarea>

                    <button type="submit" className="pure-button pure-button-primary edit-recipe-submit"
                            disabled={isLoading}
