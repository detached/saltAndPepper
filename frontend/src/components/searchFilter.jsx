...
function filterStatesReducer(state, action) {
    let mapPossibleFilterToCheckboxStates = (key) => {
        ...
        if (newValues) {
            ...
            newValues.forEach((filterValue) => {
                const existingState = state[key] ? state[key][filterValue.value]?.checked : null;
    ...
    switch (action?.type) {
        case FilterStatesAction.NEW_FILTER_VALUES:
            ...
            newFilterValueStates[FilterKey.AUTHOR] = mapPossibleFilterToCheckboxStates(FilterKey.AUTHOR);
            newFilterValueStates[FilterKey.CATEGORY] = mapPossibleFilterToCheckboxStates(FilterKey.CATEGORY);
            newFilterValueStates[FilterKey.CUISINE] = mapPossibleFilterToCheckboxStates(FilterKey.CUISINE);
...
export function SearchFilter({ possibleFilter, onSelectedValueChanged }) {
    ...
    const [filterStates, dispatchFilterStates] = useReducer(filterStatesReducer, {});
    ...
    useEffect(() => {
        ...
        let selectedValues = {};
        selectedValues[FilterKey.AUTHOR] = filterOnlySelectedValues(filterStates[FilterKey.AUTHOR]);
        selectedValues[FilterKey.CATEGORY] = filterOnlySelectedValues(filterStates[FilterKey.CATEGORY]);
        selectedValues[FilterKey.CUISINE] = filterOnlySelectedValues(filterStates[FilterKey.CUISINE]);
        onSelectedValueChanged(selectedValues);
    }, [filterStates, onSelectedValueChanged]);

    const toggleMenuActive = useCallback((key) => {
        if (activeMenu === key) {
            setActiveMenu("");
        } else {
            setActiveMenu(key);
    ...
    const toggleCheckbox = useCallback((key, value, checked) => {
        dispatchFilterStates({
    ...
    function dropdown(key, values) {
        ...
        return <li key={key} id={key} className={menuClassNames}>
            <p className={titleClassNames} onClick={() => toggleMenuActive(key)}>{t("search.filter." + key)}</p>
            <ul className="searchfilter-children-box pure-menu-children">
                {
                    Object.entries(values).map((item) => {
                        const filterValue = item[0];
                        const filterState = item[1];
                        return <li className="pure-menu-item pure-form" key={filterValue}>
                            <label>
                                <input type="checkbox" 
                                    value={filterValue} 
                                    defaultChecked={filterState.checked} 
                                    onChange={(e) => toggleCheckbox(key, filterValue, e.target.checked)} 
                                    /> {filterState.label}
                            </label>
                        </li>
                    })
