...
class CommentsController(
    ...
    @Get("/recipe/{recipeId}")
    fun getAllCommentsOfRecipe(@PathVariable("recipeId") recipeId: String): CommentsViewModel {
        val allComments = commentsService.getAllComments(RecipeId(recipeId))

        return CommentsViewModel(
            recipeId = recipeId,
            comments = allComments.map { it.toVO() },
        )
    }
    ...
    private fun Comment.toVO() = CommentViewModel(
        id = id.id,
